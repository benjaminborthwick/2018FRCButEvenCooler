...
public class XboxController extends Joystick {
	...
	public boolean getLeftTrigger() {
		return (Math.abs(getRawAxis(Axis_LeftTrigger)) > 0.1);
	}

	public boolean getRightTrigger() {
		return (Math.abs(getRawAxis(Axis_RightTrigger)) > 0.1);
	}
	...
	public double getLeftXAxis() {
		if (Math.abs(getRawAxis(Axis_LeftX)) > .2) {
	...
	public double getLeftYAxis() {
		if (Math.abs(getRawAxis(Axis_LeftY)) > .2) {
	...
	public double getRightXAxis() {
		if (Math.abs(getRawAxis(Axis_RightX)) > .2) {
	...
	public double getRightYAxis() {
		if ( Math.abs(getRawAxis(Axis_RightY)) > .2) {
